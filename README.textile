...
Requirements:
Java Runtime Environment or SDK 1.8.0 or better.
MySQL or PostgreSQL database accessible from the machine running the server.

Running JavaTrek:
1. Download and install a compatible version of the Java Runtime Environment or SDK.  (See above.)
2. Create your MySQL or PostgreSQL database and note its server, username, password and database name.
3. Execute the appropriate jtrek_tables.sql-xxxxxxx file inside the database so that the table structure is created.
4. Make a copy of "jtrek.properties-postgresql" and name it "jtrek.properties".  Update the jdbc.driver, jdbc.url, jdbc.user and jdbc.password fields.  Save the file.
